	...
	subscribeToStream : function(){
		...
		conn.login('magicmirror@demoengg.sdo', 'Salesforce1', function(err, res) {
			...
			conn.streaming.topic("QM_Report_Stream").subscribe(function(message) {
			  	conn.query('Select Id, Name, APAC__c, AMER__c, EMEA__c, APAC_hrs__c, AMER_hrs__c, EMEA_hrs__c, SYD_hrs__c, APAC_ANZ_usd__c, APAC_IND_usd__c, AMER_SF_usd__c, AMER_CHI_usd__c, EMEA_usd__c, Remarks__c, CreatedDate from QM_Task__c order by createddate desc limit 1', function(err, res) {
					if (err) { return console.error(err); }
					console.log(res);
